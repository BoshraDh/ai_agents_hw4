...
def _make_response(prompt_tokens: int = 10, completion_tokens: int = 5, text: str = "ok"):
    usage = MagicMock()
...
def test_chat_complete_returns_text(monkeypatch):
    with patch("hw4.shared.gatekeeper.openai.OpenAI") as MockClient:
        ...
        mock_client.chat.completions.create.return_value = _make_response(text="hello")
...
def test_reset_counts(monkeypatch):
    with patch("hw4.shared.gatekeeper.openai.OpenAI") as MockClient:
        mock_client = MockClient.return_value
        mock_client.chat.completions.create.return_value = _make_response(10, 5)
        from hw4.shared.gatekeeper import ApiGatekeeper
        gk = ApiGatekeeper()
        gk.chat_complete([{"role": "user", "content": "x"}], "model", 100)
        gk.reset_counts()
